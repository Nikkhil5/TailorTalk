...
def extract_slots(text: str, timezone: str = "Asia/Kolkata") -> dict:
    ...
    # Remove filler/booking verbs to avoid parser confusion
    clean_text = re.sub(
        r'\b(?:yes|please|book|schedule|appointment|meeting|call|wanna|want to|can you|for|between)\b',
        '', text_lower, flags=re.IGNORECASE
    ).strip()

    # Handle 24-hour formats like 15:00 or 1530
    clean_text = re.sub(
    ...
    # Handle vague phrases (map to business hours)
    time_map = {
        "morning": "10:00 AM",
        "afternoon": "2:00 PM",
        "evening": "5:00 PM",
    ...
    for vague, specific in time_map.items():
        if vague in clean_text:
            clean_text = re.sub(vague, specific, clean_text, flags=re.IGNORECASE)
            break

    # Fix formats like 3pm -> 3 PM
    clean_text = re.sub(r'(\d+)([ap]m)', r'\1 \2', clean_text, flags=re.IGNORECASE)

    # Add default context
    if not any(day in clean_text for day in [
        "mon", "tue", "wed", "thu", "fri", "sat", "sun", "today", "tomorrow", "week", "month"
    ]):
    ...
    print("[DEBUG] Final text to parse:", repr(clean_text))
...
def suggest_alternative(slots: dict) -> str:
    """Suggest cleaner fallback slots"""
    try:
        start_dt = datetime.fromisoformat(slots["start"])
        business_start = 9
        business_end = 18
...
def _format_time_friendly(datetime_str: str) -> str:
    try:
        ...
        now = datetime.now(dt.tzinfo or pytz.timezone("Asia/Kolkata"))
        if dt.date() == now.date():
